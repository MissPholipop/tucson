...
public class IndexManager{
    ...
    static Logger log = LoggerFactory.getLogger(IndexManager.class);
    /**
     * 创建索引管理器
     * @return 返回索引管理器对象
     */
     ...
     */
    @SuppressWarnings("deprecation")
	public static List<SearchDto> searchIndex(String text){
        Date date1 = new Date();
        List<SearchDto> test = new ArrayList<SearchDto>();
        try{
            directory = FSDirectory.open(new File(INDEX_DIR));
            analyzer = new StandardAnalyzer(Version.LUCENE_CURRENT);
           ...
           // Map map = new HashMap();
            ...
            for (int i = 0; i < hits.length; i++) {
               ...
               // sdto.setTitle(hitDoc.get("title"));
                sdto.setContent(hitDoc.get("content"));
                sdto.setFilePath(hitDoc.get("path"));
                test.add(sdto);
    ...
    public static void main(String[] args){
     /*   File fileIndex = new File(INDEX_DIR);
       ...
       System.out.println(searchIndex("man")); 
    }
}